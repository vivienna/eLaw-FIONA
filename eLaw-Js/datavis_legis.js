...
function Map(data) {
    ...
    showDetails = function (d, i) {
        ...
        if (!d.ruling) {
            ...
            //onclick=printDiv('PR" & SecNo.Replace(".", "").Replace(" ", "_") & "')
            content = "<p class='main' ><div onclick=RedirectId('"+ d.caseID +"') style='cursor:pointer; color:blue;'>" + d.caseName + "</div></span></p>";
            content += '<hr class="tooltip-hr">';
    ...
    showLinkDetails = function (d, i) {
        ...
        content = '<p class="main"> Applied </span></p>';
        //content = '<p class="main"> Legislation </span></p>';
        tooltip.showTooltip(content, d3.event);
    }
...
function init(caseID) {
          ...
          if ( checkloopno>24)
            ...
            $('#datatbl').find("#" + v).each(function () {
                ...
                data.referredCases.push(referredCase);
                 ...
                 var caseLinks1 = " <div class='boxRecentResult1'><h2 class='title2Case2'><a href='#' onclick=RedirectId('"+ caseID +"')  id='" + caseID + "'  data-title=''>" + caseName + "</a></h2>"
                    var caseLinks= " <p class='title3Case' ><a href='#' onclick=RedirectId('"+ caseID +"')>" + court + "</a></p></div>"
    ...
    $.getData = function () { /////////////////////////////////////////////////////////////////////// to be removed 
        return $.ajax({
            url: 'precMapLoader_legis.aspx?id=' + caseID +"&t="+$('#orientation').html().trim(),
            dataType: 'html'
        }).promise();
    };
...
///Redirect function to correct page 17-10-2013
function RedirectId(id)
{
var cl= document.getElementById("CL").innerHTML;
if (cl==1)
  {
   window.location = "legislationSectionDisplayed.aspx?id="+id
  }
  else{ window.location = "case_notes/showcase.aspx?id="+id }
