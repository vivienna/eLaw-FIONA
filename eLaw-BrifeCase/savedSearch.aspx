<%@ Page Language="VB" AutoEventWireup="false" CodeFile="SavedSearch.aspx.vb" Inherits="SavedSearch" %>
...
<link id="Link2" runat="server" rel="icon" href="../GUI/favicon.ico" type="image/ico"/>
    ...
    <script>
            ...
            function delBtn(obj) {
                //            alert("works");
                if (confirm("are you sure you want to delete " + $(obj).attr('value') + " ?"))
                //$.post("savedSearch.aspx", { 'dl': ($(obj).attr('id').toString()) });

                    $.ajax({
                        type: "POST",
                        url: "savedSearch.aspx",
                        data: { 'dl': ($(obj).attr('id').toString()) },
                        //contentType: "application/json; charset=utf-8",
                        //dataType: "html",
                        success: function (data) {
                            $('#refreshData').click();
                            //$('#updateP').html(data.children('#updateP').html());
                        },
            ...
            function delSel() {
                //            alert("works");
                if (confirm("are you sure you want to delete all the selected Search ?")) {
                    var times = "" ;
                    $('[name="checkToDel"]:checked').each(function () {
                        times += $(this).val()+";";
                    });
                    times = times.substr(0, times.length - 1);
                    //alert(times);
                    $.post("savedSearch.aspx", { 'dl': (times) });
                }
            }

//            $(document).ready(function () {
//                $('#selectall').click(function () { 
//                     $('[name="checkToDel"]').prop('checked', $(this).attr('checked')?true:false);
//                 });
//                 $('[name="checkToDel"]').click(function () {
...
//            });
   ...
   <title>Saved Searches | myBriefcase | eLaw.my</title>
</head>
<body>
...
<div id="content3">
    ...
    <div class="clear"></div>
		<nav class="wrapperTabs">
    		...
    		<li class="current1"><a href="#">SAVED SEARCHES</a></li> 
    	...
    	<!--<a style="cursor:pointer" onclick="delSel()"><img border="0" src="../img/icoDelete.gif"/>Delete</a>--> <!--onclick=GetValue() ?? -->
        ...
        <a href="#"><img border="0" src="../img/icoSave2.gif"/>Save<a href="#"><img border="0" src="../img/icoSave2.gif"/>Save</a>
        <a href="#"><img border="0" src="../img/icoWords.gif"/>Words</a>-->
    	<!--<asp:LinkButton ID="LinkButton1" runat="server" Text="PDF"><img border="0" src="../img/icoPdf.gif"/>PDF</asp:LinkButton>-->
        ...
        <asp:ScriptManager runat="server" ID="srptmgr"></asp:ScriptManager>
        <asp:UpdatePanel id="updateP" runat="server" UpdateMode="Conditional">
            <ContentTemplate>
    	    <aside class="boxBgblue2 >            
              <span class="tTextNorm2" style="font-size:14px">List of saved Search</span>
              <span style="margin-left:20px">
                <asp:DropDownList ID="SearchType" runat="server" AutoPostBack="True" class="advBoxS6" style="width:100px;"> 
                    <asp:ListItem value="0" Selected="True">Cases</asp:ListItem> 
                    <asp:ListItem value="1">Legislation</asp:ListItem>
                </asp:DropDownList>
            ...
            <asp:Button ID="refreshData" runat="server" style="display:none" />
